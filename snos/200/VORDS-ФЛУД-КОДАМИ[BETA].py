...
number = int(input('Введите номер телефона жертвы: '))
count = 0
nomer = number
...
try:
    while True:
        ...
        response5 = requests.get('https://telegram.org/support?setln=ru', headers=headers)
        response6 = requests.get('https://telegram.org/support?setln=ru', headers=headers)
        print(number)
        count += 1
